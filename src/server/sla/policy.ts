...
const ruleRowSchema = z.object({
  low: z.enum(ticketPriorityValues),
  medium: z.enum(ticketPriorityValues),
  high: z.enum(ticketPriorityValues),
  critical: z.enum(ticketPriorityValues),
});
...
export const slaPolicySnapshotSchema = z.object({
  ...
  impactUrgencyRules: z.object({
    low: ruleRowSchema,
    medium: ruleRowSchema,
    high: ruleRowSchema,
    critical: ruleRowSchema,
  }),
  warningMinutes: z.number().int().nonnegative(),
  pauseStatuses: z.array(z.enum(ticketStatuses)),
  reopenBehavior: z.object({
    response: z.enum(["reset", "preserve"]),
    resolution: z.enum(["reset", "preserve"]),
  }),
...
export type StoredSlaPolicy = {
  ...
  holidays: unknown;
  impactUrgencyRules: unknown;
  warningMinutes: number;
  pauseStatuses: unknown;
  reopenBehavior: unknown;
  responseTargets: unknown;
...
export function snapshotSlaPolicy(policy: StoredSlaPolicy | null | undefined): SlaPolicySnapshot {
  ...
  return slaPolicySnapshotSchema.parse({
    ...
    supportHours: policy.supportHours,
    holidays: policy.holidays,
    impactUrgencyRules: policy.impactUrgencyRules,
    warningMinutes: policy.warningMinutes,
    pauseStatuses: policy.pauseStatuses,
    reopenBehavior: policy.reopenBehavior,
    responseTargets: policy.responseTargets,
    resolutionTargets: policy.resolutionTargets,
  });
}
...
function windowsForDate(
  ...
  return policy.supportHours[parts.weekday]
    .map((window) => {
      const [startHour, startMinute] = window.start.split(":").map(Number);
      const [endHour, endMinute] = window.end.split(":").map(Number);
      const start = utcForLocal(
        policy.timezone,
        date.year,
        date.month,
        date.day,
...
export function addSupportMilliseconds(
  start: Date,
  durationMs: number,
  policy: SlaPolicySnapshot,
): Date {
  if (!Number.isFinite(start.getTime()) || !Number.isFinite(durationMs) || durationMs < 0)
    throw new Error("Invalid SLA time input.");
...
export function subtractSupportMilliseconds(
  end: Date,
  durationMs: number,
  policy: SlaPolicySnapshot,
) {
  if (!Number.isFinite(end.getTime()) || !Number.isFinite(durationMs) || durationMs < 0)
    throw new Error("Invalid SLA time input.");
  let remaining = durationMs;
  let cursor = new Date(end);
  for (let dayOffset = 0; dayOffset < 3700; dayOffset += 1) {
    const local = localParts(cursor, policy.timezone);
    const windows = windowsForDate(policy, local).reverse();
    for (const window of windows) {
      if (cursor <= window.start) continue;
      if (cursor > window.end) cursor = window.end;
      const available = cursor.getTime() - window.start.getTime();
      if (remaining <= available) return new Date(cursor.getTime() - remaining);
      remaining -= available;
      cursor = window.start;
    }
    const startOfDay = utcForLocal(policy.timezone, local.year, local.month, local.day, 0, 0);
...
export function calculateSlaDeadlines(
  ...
  return {
    responseDueAt: addSupportMinutes(start, policy.responseTargets[priority], policy),
    resolutionDueAt: addSupportMinutes(start, policy.resolutionTargets[priority], policy),
  };
}

export function calculateReopenSla(
  reopenedAt: Date,
  priority: TicketPriority,
  policy: SlaPolicySnapshot,
  current: { responseDueAt: Date | null; respondedAt: Date | null; resolutionDueAt: Date | null },
) {
  const reset = calculateSlaDeadlines(reopenedAt, priority, policy);
  return {
    responseDueAt:
      policy.reopenBehavior.response === "reset" ? reset.responseDueAt : current.responseDueAt,
    respondedAt: policy.reopenBehavior.response === "reset" ? null : current.respondedAt,
    resolutionDueAt:
      policy.reopenBehavior.resolution === "reset"
        ? reset.resolutionDueAt
...
function clockState(
  ...
  const warningAt = subtractSupportMilliseconds(dueAt, policy.warningMinutes * 60_000, policy);
  return now >= warningAt ? "at_risk" : "on_track";
}

export function evaluateSla(input: {
  now: Date;
  status: TicketStatus;
  policy: SlaPolicySnapshot | null;
  ...
  const pending = [
    [response, input.responseDueAt],
    [resolution, input.resolutionDueAt],
  ]
    .filter(([state]) => state === "on_track" || state === "at_risk")
    .map(([, due]) => due as Date)
  ...
  return {
    ...
    resolution,
    nextDeadline,
    warningAt: nextDeadline
      ? subtractSupportMilliseconds(
          nextDeadline,
          input.policy.warningMinutes * 60_000,
          input.policy,
        )
      : null,
  };
