...
- **Date:** 2026-09-04
- **Owners:** Product / Engineering
- **Context:** Remote-management identity and current telemetry change on a different lifecycle from resort ownership, placement, responsibility, commercial context, and ticket history. Approximate device matching could silently assign the wrong business asset.
...
## ADR-019: Keep Level ticket actions fail-closed until a concrete trigger is available and approved

- **Status:** Accepted
- **Date:** 2026-09-08
- **Owners:** Product / Engineering
- **Context:** Step 25 permits only Level actions that are documented, available in this tenant, and explicitly approved. Current official documentation provides an automation-webhook catalogue and trigger endpoint, but publishes no stable device deep-link contract. The live tenant catalogue returned zero automation webhook triggers. Existing decisions approve only a read-only API key and approve no exact automation, write credential, high-risk action, mandatory step-up policy, or manager-approval workflow.
- **Decision:** Keep the Level action boundary fail-closed. Do not add a guessed deep link, generic action runner, dormant write client, write credential, browser action form, execution ledger, or worker handler while the intersection of documented, tenant-available, and explicitly approved actions is empty. Activation requires a concrete tenant trigger with authorization required, an exact decision-log allowlist, a separate server-only write key, defined eligible roles and risk tier, approved confirmation and step-up or manager-approval policy, and reviewed timeout, unknown-result, reconciliation, and audit behavior.
- **Consequences:** Step 25 is blocked and its checkpoint cannot truthfully pass today. Tickets and read-only Level context continue to work unchanged. No browser or provider write surface exists to test for action submission, so the existing permission matrix and fail-closed device-URL tests remain the executable boundary. When a trigger is created and approved, Step 25 must be rerun and must add focused permission, allowlist, duplicate, timeout, replacement-device, confirmation, approval, reconciliation, and audit-completeness tests before activation.
- **Evidence:** Live `GET /v2/automations/webhooks?limit=100` on 2026-09-08 returned HTTP 200, `has_more: false`, and zero entries; current Level-owned automation-trigger and device-navigation documentation; `docs/integrations/level.md`; ADR-015; ADR-017; `docs/role-permission-matrix.md`; and the Step 25 status record.
- **Supersedes:** None.

## ADR-020: Keep knowledge content private, versioned, and audience-filtered at the server

- **Status:** Accepted
- **Date:** 2026-09-08
- **Owners:** Product / Engineering
- **Context:** Resort staff need reusable fixes, while technician procedures and ticket resolutions can contain operational or personal information. Search result hiding alone cannot protect direct URLs, and unrestricted HTML or automatic resolution copying would create stored-script and privacy risks.
