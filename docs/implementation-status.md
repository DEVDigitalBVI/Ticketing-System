...
| 13. Make urgency and service targets measurable          | Apply deterministic priority and SLA targets without rewriting ticket history. | In progress      | Core ticket lifecycle, technician/requester workspaces, and a runnable PostgreSQL verification target.                      | Versioned immutable policy records, ticket snapshots/deadlines, configured P1–P4 matrix, timezone-aware support-calendar arithmetic, waiting-state pause/resume, reopen behavior, first-response tracking, background-safe evaluation, live queue indicators, and requester expectations are implemented. Prisma validation, formatting, lint, type checking, 18 test files/98 tests, and the production build passed on 2026-09-01. Migration-backed verification remains blocked because `127.0.0.1:54329` is not accepting connections.                     |
...
- Clock-controlled coverage proves priority rules, time zones, support hours, weekday rollover, weekends, holidays, pause/resume, immutable policy snapshots, reopen modes, and exact warning/breach boundaries. No SLA function reads the server's local timezone or current clock implicitly.
- Verification on 2026-09-01: `pnpm db:validate`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (18 files/98 tests), and `pnpm build` passed. The guarded `pnpm test:database` check was attempted but could not connect to the configured local test PostgreSQL service at `127.0.0.1:54329`; no Docker or `psql` executable is available in this environment to start it.
