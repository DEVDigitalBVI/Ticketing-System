...
| 4. Establish the persistent data foundation            | Create a safe PostgreSQL and Prisma baseline before domain persistence.     | Complete         | ADR-007 and ADR-008.                                                                                 | PostgreSQL 17.11 Compose infrastructure, Prisma 7.10, private `service_desk` schema, initial migration, fictional idempotent seed, guarded reset/test workflows, repositories, server/public environment validation, and six database constraint tests added. Clean development and test databases were migrated, seeded, tested, and reset on 2026-08-31. The same empty foundation was migrated and verified in the approved Supabase project.                                         |
...
- Added fictional, idempotent seed data for one organisation, two properties, three departments, three roles, three users, property-scoped memberships, and one immutable audit event.
- Verified a clean isolated development database with migrate, seed, guarded schema reset, remigrate, and reseed. Verified the separate test database with reset, migrate, seed, and 6 passing database constraint tests.
