...
## Step 22 implementation evidence

- Added one optional tenant-safe primary-asset relationship to tickets. Technician ticket detail resolves device context through that relationship and the asset's current `level` external link; replacement links therefore select the replacement snapshot without hostname matching.
- Added a server-only context boundary guarded by property-scoped `level.context.read`. Its returned shape excludes raw payloads, credentials, command output, network telemetry, checksums, serials, match metadata, provider errors, and internal error codes. Requesters fail authorization before database access.
- Ticket and asset rendering use only local synchronized records. Context-query failure, a newer failed or partial run, a failed snapshot, or a snapshot more than two hours old produces a clear degraded or stale treatment while leaving the parent page usable. The previous successful timestamp is preserved separately from later failed attempts.
- Current official Level Device Listing and Device Overview documentation was rechecked on 2026-09-08. It publishes no stable identifier-based device URL, so the device ID is shown and the link allowlist remains intentionally empty. Group and selected alerts are labelled as not synchronized because Step 21 did not approve or store those fields.
- Focused tests cover technician authorization, requester denial, stale snapshots, missing links, provider and query failure, fail-closed URL handling, and replacement devices. `pnpm db:validate`, formatting, lint, strict type checking, all 35 application test files/185 tests, and the production build pass. The guarded database suite could not start because `TEST_DATABASE_URL` is not configured; it did not touch the application database.
...
- Supabase project `zwcmljkjoxrfzfyphdtc` is active and healthy. On 2026-09-04, the reviewed migrations for Steps 7, 8, 13, 14, 15, 17, and 21 were applied in dependency order. Deployment exposed and corrected two previously unexercised migration defects: the Step 7 seed now references the existing resort property UUID, and Step 8 creates the composite candidate keys required by its tenant-consistent foreign keys. The private attachment bucket and Level identity constraint were verified after deployment.
- Live tenant activation on 2026-09-04: the server-stored Level credential passed `GET /v2/devices?limit=1` on the first attempt with HTTP 200, and the guarded pagination iterator completed against the nonempty tenant inventory. Only safe request metadata and an aggregate count were observed; no production device payload was written to logs, tests, documentation, or source control. The first durable snapshot remains pending because the configured PostgreSQL runtime at `127.0.0.1:54329` is offline.
...
- `LEVEL_API_KEY` extends the established server environment secret convention and is never exposed with a browser-safe prefix. The repository contains only an empty example value. The server-configured key passed the approved live health request and guarded pagination on 2026-09-04 without persisting or logging device payloads.
...
- Final verification on 2026-09-04: `pnpm check` passed, including Prisma generation/validation, formatting, ESLint, strict TypeScript, 21 test files/115 tests, and the Next.js production build. The guarded `pnpm test:database` run was attempted and stopped at `ECONNREFUSED 127.0.0.1:54329`; no local PostgreSQL service is running. After the prerequisite migration chain was approved and deployed, the hosted metadata table and private bucket were verified.
