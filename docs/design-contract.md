...
| `/knowledge`        | Staff and technicians              | Search-led editorial cards for published guidance, with audience-aware author controls, article reading, review metadata, ticket linking, and responsive single-column transitions.      |
| `/assets`           | Technician and IT management       | Property-scoped inventory register with readable current location, custodian, type, criticality, and lifecycle state; management actions use the same panels and field hierarchy.        |
| `/health`           | Operations                         | Branded application-shell health view. It does not claim that external services are healthy.                                                                                             |
| `/admin/operations` | Authorized IT operations           | Shared-shell dependency health view with compact semantic status cards, failure-management links, and safe diagnostic records.                                                           |
...
- Asset inventory uses a compact ledger-style list and two-column detail/history composition at desktop widths, collapsing to the established single-column mobile hierarchy without adding a second visual system.
...
| File storage           | Private Supabase Storage integrated             | Staff and technician ticket context panels retain their hierarchy and now include compact upload/list controls. Object access remains server-authorized; quarantined or infected states are expressed with text as well as color.                     |
...
| Level.io               | Read-only provider boundary                     | The administrator configuration page reports server-key status and can perform a one-record health read. The selected-ticket device panel remains unavailable; there is no live device lookup, synchronization, deep link, webhook, or remote action. |
