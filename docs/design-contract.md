...
- Monitoring status and Level.io context still render explicit unavailable states. Formal SLA-derived at-risk and breached queue views remain placeholders until SLA timing is approved. `/new-ticket`, `/my-tickets`, and `/technician` may use real authorized data, but they must continue to preserve the approved layout and avoid demonstration records.
