...
2. What are the server-only SMTP host, port, TLS mode, username, password, and approved sender address? The adapter is complete, but no email or initial administrator can be provisioned until these values are supplied outside Git.
